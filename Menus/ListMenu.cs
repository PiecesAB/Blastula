...
namespace Blastula.Menus
{
	...
	public partial class ListMenu : BaseMenu
	{
		...
		/// <summary>
		/// When positive, the minimum frame time of holding the button until it starts selecting menu options automatically.
		/// (Leave it non-positive to disable this autoscrolling.)
		/// </summary>
		[ExportGroup("Autoscroll")]
		[Export] public int autoscrollBeginFrames = 0;
		/// <summary>
		/// When autoscrollBeginFrames is positive, the time between selections while holding the button.
		/// </summary>
		[Export] public int autoscrollInterval = 6;

		protected List<ListNode> menuNodes = new List<ListNode>();

		private int selectFramesRemaining = -1;

		public int selection { get; private set; } = 0;

		public ListNode BaseGetSelectionNode()
			 => selection >= 0 ? menuNodes[selection] : null;
		...
		private bool HitButton(string inputName)
			=> InputManager.ButtonPressedThisFrame(inputName)
			|| (autoscrollBeginFrames > 0
				&& InputManager.GetButtonHeldFrames(inputName) is ulong h
				&& h >= (ulong)autoscrollBeginFrames
				&& (h - (ulong)autoscrollBeginFrames) % (ulong)autoscrollInterval == 0);
		...
		private bool HitPrevButton()
		{
			switch (direction)
			{
				...
				default:
					return HitButton("Menu/Up");
				case Direction.Horizontal:
					return HitButton("Menu/Left");
			}
		}
		...
		private bool HitNextButton()
		{
			switch (direction)
			{
				case Direction.Vertical: 
				default:
					return HitButton("Menu/Down");
				case Direction.Horizontal:
					return HitButton("Menu/Right");
			}
		}

		protected virtual void HighlightChanged(int oldSelection, int newSelection)
		{
		...
		public override void _Process(double delta)
		{
			...
			if (oldSelection != selection)
			{
				...
				HighlightChanged(oldSelection, selection);
			}
		}
	}
}
